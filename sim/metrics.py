...
def compute_metrics(nav: pd.DataFrame, trades: pd.DataFrame, benchmarks: list, initial_cash: float) -> dict:
    ...
    m["total_return"] = final / initial_cash - 1.0
    m["total_pnl"] = final - initial_cash
    cal_days = (idx[-1] - idx[0]).days
    m["cagr"] = (final / initial_cash) ** (365.0 / cal_days) - 1.0 if cal_days >= 30 and final > 0 else None
    ...
    if len(navs) > 1:
        m["daily_change"] = float(navs.iloc[-1] - navs.iloc[-2])
        m["daily_change_pct"] = float(navs.iloc[-1] / navs.iloc[-2] - 1.0)
    else:
    ...
    # 月次リターン
    monthly = []
    grp = navs.groupby(navs.index.to_period("M"))
...
def series_for_chart(nav: pd.DataFrame, benchmarks: list, initial_cash: float) -> dict:
    ...
    out = {
        ...
        "port": [round(v / initial_cash * 100, 3) for v in navs],
        "nav": [round(v) for v in navs],
        "drawdown": [round(v * 100, 3) for v in drawdown(navs)],
